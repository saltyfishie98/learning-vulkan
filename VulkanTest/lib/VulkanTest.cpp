...
void VulkanTest::framebufferResizeCallback(GLFWwindow* window,
										   int width,
										   int height) {
...
void VulkanTest::populateDebugMessengerCreateInfo(
...
SwapChainSupportDetails
	VulkanTest::querySwapChainSupport(VkPhysicalDevice device) {
...
VkSurfaceFormatKHR VulkanTest::chooseSwapSurfaceFormat(
...
VkPresentModeKHR VulkanTest::chooseSwapPresentMode(
...
VkExtent2D
	VulkanTest::chooseSwapExtent(const VkSurfaceCapabilitiesKHR& capabilities) {
...
void VulkanTest::createGraphicPipeline() {
...
void VulkanTest::cleanupSwapChain() {
...
uint32_t VulkanTest::findMemoryType(uint32_t typeFilter,
...
void VulkanTest::createBuffer(VkDeviceSize size,
...
void VulkanTest::copyBuffer(VkBuffer srcBuffer,
							VkBuffer dstBuffer,
							VkDeviceSize size) {
...
void VulkanTest::createDescriptorSetLayout() {
...
void VulkanTest::createUniformBuffers() {
...
void VulkanTest::updateUniformBuffer(uint32_t currentImage) {
...
void VulkanTest::createDescriptorSets() {
...
void VulkanTest::createTextureImage() {
	...
	stbi_uc* pixels = stbi_load("../VulkanTest/textures/texture.jpg", &texWidth,
...
void VulkanTest::createImage(uint32_t width,
							 uint32_t height,
							 VkFormat format,
...
void VulkanTest::transitionImageLayout(VkImage image,
									   VkFormat format,
...
void VulkanTest::copyBufferToImage(VkBuffer buffer,
								   VkImage image,
								   uint32_t width,
								   uint32_t height) {
	VkCommandBuffer commandBuffer = beginSingleTimeCommands();
...
void VulkanTest::frameRate() {
	...
	auto elapsed_nanoseconds = endTime - startTime;
	if (elapsed_nanoseconds.count() >= 1000000000) {
		std::cout << "FPS: " << count - 1 << "\n";
		count = 0;
		resetTimer = true;
	}
}
